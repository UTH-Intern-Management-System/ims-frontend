...
const analyticsService = {
  ...
  getInternPerformanceAnalytics: () => {
    const totalInterns = mockInterns.length;
    const activeInterns = mockInterns.filter(
    ...
    mockAssessments.forEach((assessment) => {
      ...
      if (intern && intern.department) {
        ...
        departmentPerformance[intern.department].total += parseFloat(
    ...
    return Promise.resolve({
      data: {
        ...
        averageGPA: (() => {
          const internsWithGPA = mockUsers.filter(
            (u) => u.role === "INTERN" && u.gpa
          );
          return internsWithGPA.length > 0
            ? internsWithGPA.reduce((sum, intern) => sum + intern.gpa, 0) /
                internsWithGPA.length
            : 0;
        })(),
      },
    });
  ...
  getTaskAnalytics: () => {
    ...
    const averageProgress =
      totalTasks > 0
        ? mockTasks.reduce((sum, task) => sum + (task.progress || 0), 0) /
          totalTasks
        : 0;
  ...
  getRecruitmentAnalytics: () => {
    const totalCampaigns = mockRecruitmentCampaigns.length;
    const activeCampaigns = mockRecruitmentCampaigns.filter(
      (c) => c.status === "active"
    ).length;
    const totalApplications = mockRecruitmentCampaigns.reduce(
      (sum, c) => sum + c.applications,
      0
    );
    const totalPositions = mockRecruitmentCampaigns.reduce(
      (sum, c) => sum + c.positions,
      0
    );

    // Applications per campaign
    const applicationsPerCampaign = mockRecruitmentCampaigns.map(
      (campaign) => ({
        campaignTitle: campaign.title,
        applications: campaign.applications,
        positions: campaign.positions,
        ratio:
          campaign.positions > 0
            ? (campaign.applications / campaign.positions).toFixed(2)
            : 0,
      })
    );
    ...
    mockRecruitmentCampaigns.forEach((campaign) => {
      departmentDistribution[campaign.department] =
        (departmentDistribution[campaign.department] || 0) + 1;
    });
    ...
    return Promise.resolve({
      data: {
        ...
        totalApplications,
        totalPositions,
        applicationToPositionRatio:
          totalPositions > 0
            ? (totalApplications / totalPositions).toFixed(2)
            : 0,
        applicationsPerCampaign,
        departmentDistribution,
        averageApplicationsPerCampaign:
          totalCampaigns > 0
  ...
  getTrainingAnalytics: () => {
    const totalPrograms = mockTrainingPrograms.length;
    const activePrograms = mockTrainingPrograms.filter(
      (p) => p.status === "active"
    ).length;
    const totalCapacity = mockTrainingPrograms.reduce(
      (sum, p) => sum + p.maxParticipants,
      0
    );
    const totalEnrolled = mockTrainingPrograms.reduce(
    ...
    // Module distribution
    const totalModules = mockTrainingPrograms.reduce(
      (sum, p) => sum + p.modules.length,
      0
    );
    const averageModulesPerProgram =
      totalPrograms > 0 ? (totalModules / totalPrograms).toFixed(1) : 0;
    ...
    return Promise.resolve({
      data: {
        ...
        averageProgramDuration:
          totalPrograms > 0
            ? mockTrainingPrograms.reduce((sum, p) => {
  ...
  getProgressAnalytics: () => {
    ...
    const totalHours = mockDailyProgress.reduce(
      (sum, p) => sum + p.hoursWorked,
      0
    );
    const averageHoursPerDay =
      totalProgressEntries > 0
        ? (totalHours / totalProgressEntries).toFixed(1)
        : 0;

    // Mood tracking
    const moodDistribution = {};
    mockDailyProgress.forEach((progress) => {
      moodDistribution[progress.mood] =
        (moodDistribution[progress.mood] || 0) + 1;
    });
    ...
    Object.keys(progressByIntern).forEach((internId) => {
      progressByIntern[internId].averageHoursPerDay = (
        progressByIntern[internId].totalHours / progressByIntern[internId].days
      ).toFixed(1);
    });
    ...
    return Promise.resolve({
      data: {
        ...
        moodDistribution,
        progressByIntern,
        mostProductiveDay: mockDailyProgress.reduce(
          (max, p) => (p.hoursWorked > max.hoursWorked ? p : max),
          { hoursWorked: 0 }
        ),
      },
    });
  ...
  getDashboardSummary: () => {
    return Promise.all([
      ...
      analyticsService.getInternPerformanceAnalytics(),
